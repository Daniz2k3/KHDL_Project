...
def Geturl(data):  # Hàm lấy link
    ...
    motas = driver.find_elements(
        By.XPATH, "//*[@class='sapo uk-hidden-small uk-hidden-medium']"
    )
    ...
    for element, title, link, mota in zip(elements, titles, link_elements, motas):
        ...
        data = pd.concat(
            [
                data,
                pd.DataFrame(
                    {"Tieu de": [element.text], "Gia": [title.text], "link": [href]}
                ),
            ],
            ignore_index=True,
        )
...
def get_square():  # Lấy diện tích
    try:  # Diện tích
        square = driver.find_element(
            By.XPATH, '*//*[*[text()="Diện tích:"]]'
        ).text.split()  # Tách lí tự
...
def get_address_tinh():  # Lấy tên tỉnh
    a = driver.find_element(
        By.XPATH, '//*[@id="post-detail"]/body/div[4]/div/div/ul/li[3]/a'
    ).text
    return a
...
def get_address_huyen():  # Lấy tên huyện
    b = driver.find_element(
        By.XPATH, '//*[@id="post-detail"]/body/div[4]/div/div/ul/li[4]/a'
    ).text
    return b
...
def get_price():  # Lấy giá ( quy đổi về tỷ)
    price = driver.find_element(
        By.XPATH,
        '//*[@id="post-detail"]/body/div[6]/div/div/div/div[2]/div/div[1]/div/div/div/div/div/div[1]/strong',
    ).text
    a = price.split()
    if a[1] == "triệu":
...
def get_loainha():  # Lấy loại nhà
    b = driver.find_element(
        By.XPATH, '//*[@id="post-detail"]/body/div[4]/div/div/ul/li[2]/a'
    ).text
    return b
...
def get_huongnha():  # Lấy hướng nhá
    try:
        huong = driver.find_element(
            By.XPATH, '*//*[*[text()="Hướng nhà:"]]'
        ).text.split()
        huong = huong[2]
    except:
        huong = ""
...
def crawl_website(url):  # hàm lấy thông tin từng cái
    title = driver.find_element(
        By.XPATH,
        "//*[@id='post-detail']/body/div[6]/div/div/div/div[2]/div/div[1]/div/div/div/h1",
    ).text
