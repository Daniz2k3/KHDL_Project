...
#  Cào dữ liệu (web scraping) bằng Selenium là quá trình sử dụng thư viện Selenium để tự động hóa trình duyệt web nhằm truy cập, thu thập và trích xuất dữ liệu từ các trang web. Selenium là một công cụ mạnh mẽ thường được sử dụng cho kiểm thử tự động các ứng dụng web, nhưng nó cũng rất hiệu quả trong việc cào dữ liệu từ các trang web động, nơi mà nội dung được tải động thông qua JavaScript.
...
while count <= 305: # page kết thúc
    try:
        ...
        title = [elem.text for elem in elems][:-5]
        links = [elem.get_attribute('href') for elem in elems][:-5]
        ...
        time = [elem.get_attribute('datetime') for elem in elems]
     ...
     # Lấy các thông tin bên trong các item
        ...
        for i in range(len(title)):
            ...
            try:
                ...
                phongwc.append(elems_phongwc.text)
            except NoSuchElementException:
                phongwc.append("")  
            try:
                ...
                diachi.append(elems_diachi.text)
            except NoSuchElementException:
                diachi.append("")  
            try:
                ...
                phongngu.append(elems_phongngu.text)
            except NoSuchElementException:
                phongngu.append("")  
            try:
                ...
                huongnha.append(elems_huongnha.text)
            except NoSuchElementException:
                huongnha.append("")
            try:
                ...
                huongbancong.append(elems_huongbancong.text)
            except NoSuchElementException:
                huongbancong.append("")      
            try:
        ...
        # Kết quả sẽ là DataFrame mới chứa thông tin từ ba DataFrame ban đầu, được gộp dựa trên cột 'idx'.
        data = df3.merge(result, on='idx')

        if count == 301:
            data.to_csv('page(301-500).csv', index=False, encoding='utf-8-sig')
        else:
            data.to_csv('page(301-500).csv', mode='a', header=False, index=False, encoding='utf-8-sig')
